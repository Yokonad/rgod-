...
const trabajadoresAvailable = ref(false);
...
// Load Trabajadores (optional module - may not exist)
async function loadTrabajadores() {
  loadingTrabajadores.value = true;
  try {
    const data = await $fetch<{ trabajadores: any[] }>('/api/modules/trabajadores/list');
    trabajadores.value = data.trabajadores || [];
    trabajadoresAvailable.value = true;
  } catch (error) {
    // Silently fail - trabajadores module is optional
    console.log('Módulo de trabajadores no disponible');
    trabajadores.value = [];
    trabajadoresAvailable.value = false;
  } finally {
